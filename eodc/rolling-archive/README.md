...
| `rolling-archive-airflow-credentials` (`poller.airflow.existingSecret`) | `AIRFLOW_BASE_URL`, `AIRFLOW_USERNAME`, `AIRFLOW_PASSWORD` | `access/rolling-archive/<poller.airflow.vaultSecretSlug>` (`url`, `username`, `password`) -- **environment-specific, unlike every other row here**: dev and prod are genuinely different Airflow instances, so e.g. `airflow-development` and `airflow-production` are two separate Vault entries, not one shared entry. Every Application manifest must set `poller.airflow.vaultSecretSlug` explicitly to the real slug for its own environment. | Poller only -- `AIRFLOW_BASE_URL` this way too when `vault.enabled: true` (`poller.airflow.baseUrl`'s plain value is only a fallback for `vault.enabled: false`), so this URL can be rotated in one place if it changes (e.g. the platform team's OpenStack/cluster migration) without a chart/manifest edit. Needs a real Airflow API user with permission to trigger DAG runs, see the gotcha below. |
| One Secret per CDSE account referenced in `poller.entries[].existingSecret` (several entries commonly share one account) -- e.g. `cdse-account1-credentials` for a 5-account production rollout, or a single `cdse-account-dev-credentials` for a slim smoke-test deployment | `username`, `password`, optional `totp_secret` | `access/rolling-archive/<account-slug>` (same field names) | Poller directly. Also the Airflow download task, IF `ROLLING_ARCHIVE_CDSE_ACCOUNTS` on the Airflow instance lists these same names (comma-separated, contiguous `CDSE_ACCOUNT1..N` order) -- `_common.py` otherwise defaults to a single `cdse-credentials` secret, which only covers a single-account local-testing setup. |
